...
export function calculateFoodShortage(state: GameState): FoodShortageResult {
  ...
  // Population is stored in compact "万" units in the prototype.
  ...
  const soldierLoss = Math.min(state.resources.army, Math.max(1, Math.ceil(shortage / 80)));
  const severity = Math.min(8, Math.max(1, Math.ceil(shortage / 5_000)));

  // MonthlyLoop already applies the base -4 peasant / -4 military shortage
  // penalty. These values are the additional severity penalty, preventing the
  // same shortage from being counted twice as a full base penalty.
  return {
    shortage,
    peasantLoss,
    soldierLoss,
    peasantSatisfactionLoss: Math.min(8, severity),
    militarySatisfactionLoss: Math.min(6, Math.ceil(severity * 0.75)),
  };
}

function applyFactionLoss(factions: GameState["factions"], id: FactionId, satisfactionLoss: number): GameState["factions"] {
  return factions.map((faction) => faction.id === id
    ? { ...faction, satisfaction: Math.max(0, faction.satisfaction - satisfactionLoss), resentment: faction.resentment + Math.ceil(satisfactionLoss / 2) }
    : faction);
}
...
export function advanceMonthWithFood(state: GameState, random?: (seed: number) => number): GameState {
  ...
  const next = settleMonth(state, random);
  ...
  const resources = { ...next.resources, army: Math.max(0, next.resources.army - shortage.soldierLoss), food: 0 };
  ...
  if (last) {
    history[history.length - 1] = {
      ...last,
      actions: [...last.actions, `粮荒：军民缺粮 ${shortage.shortage}，损失农民 ${shortage.peasantLoss} 万、士兵 ${shortage.soldierLoss} 人`, `粮荒额外导致百姓满意度 -${shortage.peasantSatisfactionLoss}、武将满意度 -${shortage.militarySatisfactionLoss}`],
